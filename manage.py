...
from donkeycar.parts.controller import WebFpv, JoystickController
from copilot.web import LocalWebController
...
def drive(cfg, model_path=None, use_joystick=False, model_type=None,
    ...
    if cfg.COPILOT_ENABLE :   
        #cfg.model_path=os.path.split(model_path)[-1].replace(".h5","").replace(".tflite","") if model_path is not None else None
        cfg.model_path=model_path if model_path is not None else None

        from copilot.stepcounter import copilot_stepcounter
        V.add(copilot_stepcounter(cfg), inputs=['cam/image_array','user/mode'], outputs=['user/mode'], threaded=True)

        from copilot.transform import copilot_transform
        V.add(copilot_transform(cfg), inputs=['cam/image_array'], outputs=['cam/image_array'], threaded=True)

        from copilot.ina219 import copilot_ina219
        V.add(copilot_ina219(cfg), threaded=True)
...
#This web controller will create a web server that is capable
    ...
    if cfg.COPILOT_ENABLE :
        from copilot.protoxyde import copilot_protoxyde
        V.add(copilot_protoxyde(cfg), inputs=['pilot/angle', 'pilot/throttle'], outputs=['pilot/angle','pilot/throttle'], threaded=True)
